...
for (const platform of ["win", "mac"]) {
  const raw = fs.readFileSync(`data_${platform}.json`, 'utf8');
  const json = JSON.parse(raw);
  const html = decrypt(json.cipher, PASSPHRASE);
  const out = `page_${platform}.html`;
  fs.writeFileSync(out, html);
  console.log(`${platform}: decrypted ${html.length} chars -> ${out}`);
  // Show first 500 chars for a sanity check
  console.log('--- head ---');
